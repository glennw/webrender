...
trait AlphaBatchHelpers {
    ...
    fn add_blend_to_batch(&self,
                          layer_index: StackingContextIndex,
                          parent_layer_index: StackingContextIndex,
                          batch: &mut PrimitiveBatch,
                          task_index: RenderTaskIndex,
...
impl AlphaBatchHelpers for PrimitiveStore {
    ...
    fn add_blend_to_batch(&self,
        ...
        match &mut batch.data {
            &mut PrimitiveBatchData::Blend(ref mut data) => {
                data.push(PrimitiveInstance {
                    global_prim_id: layer_index.0 as i32,
                    prim_address: GpuStoreAddress(0),
                    task_index: task_index.0 as i32,
                    clip_task_index: -1,                // todo: ?????????????????
                    layer_index: parent_layer_index.0 as i32,
    ...
    fn add_composite_to_batch(&self,
                              ...
                              z_sort_index: i32) {
        match &mut batch.data {
            &mut PrimitiveBatchData::Composite(ref mut data) => {
                data.push(PrimitiveInstance {
                    global_prim_id: -1,
                    prim_address: GpuStoreAddress(0),
                    task_index: task_index.0 as i32,
...
// TODO:!!!!!!!!!!!!!!!!!!!!!!!!!!! remove task_index as it's not needed!!!!!!!!!!!!!!
...
//println!("alpha_batcher: build {} tasks", self.tasks.len());
        ...
        for task in &mut self.tasks {
            ...
            //println!("\t{} opaque {} alpha", task.alpha_items.len(), task.opaque_items.len());
            ...
            for item in &task.alpha_items {
                let batch_key = match item {
                    &AlphaRenderItem::Blend(sc_index, ..) => {
                        ...
                        let needs_clipping = false; /// TODO:!!!!!!!!!!!!!!!!!! layer.clip_task.is_some();
                    ...
                    &AlphaRenderItem::Composite(..) => {
                        //println!("\t\tcomposite");
                        AlphaBatchKey::new(AlphaBatchKind::Composite,
                                           AlphaBatchKeyFlags::empty(),
                                           BlendMode::Alpha,
                                           BatchTextures::no_texture())
                    }
                ...
                if existing_alpha_batch_index == alpha_batches.len() {
                    ...
                    let new_batch = match item {
                        &AlphaRenderItem::Composite(..) => {
                            PrimitiveBatch::new(AlphaBatchKind::Composite, batch_key)
                        }
                        &AlphaRenderItem::Blend(..) => {
                            PrimitiveBatch::new(AlphaBatchKind::Blend, batch_key)
                        }
                ...
                match item {
                    &AlphaRenderItem::Composite(backdrop_id, src_id, info, z) => {
                        ctx.prim_store.add_composite_to_batch(batch,
                                                              task_index,
                    ...
                    &AlphaRenderItem::Blend(parent_sc_index, sc_index, src_id, info, z) => {
...
enum AlphaRenderItem {
    ...
    Blend(StackingContextIndex, StackingContextIndex, RenderTaskId, LowLevelFilterOp, i32),
...
impl PrimitiveBatch {
    fn new(batch_kind: AlphaBatchKind,
        ...
        let data = match batch_kind {
            ...
            AlphaBatchKind::Blend => PrimitiveBatchData::Blend(Vec::new()),
            AlphaBatchKind::Composite => PrimitiveBatchData::Composite(Vec::new()),
                ...
                PrimitiveRunCmd::PopStackingContext => {
                    ...
                    match layer.composite_kind {
                        ...
                        CompositeKind::Simple(info) => {
                            let parent_sc_index = *sc_stack.last().unwrap();
                            let mut prev_task = alpha_task_stack.pop().unwrap();
                            let item = AlphaRenderItem::Blend(parent_sc_index, sc_index, current_task.id, info, next_z);
                            next_z += 1;
                        ...
                        CompositeKind::Complex(info) => {
                            ...
                            let mut prev_task = alpha_task_stack.pop().unwrap();
                            let item = AlphaRenderItem::Composite(readback_task.id, current_task.id, info, next_z);
                            next_z += 1;
                            prev_task.as_alpha_batch().alpha_items.push(item);
                            prev_task.children.push(current_task);
